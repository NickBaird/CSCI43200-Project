...
async function initialize() {
    ...
    publicK = (await database.ref("/users/" + auth.currentUser.uid + "/public").get()).val();
...
async function add_to_map(uid) {
    if(!map.hasOwnProperty(uid)) {
        ...
        const clientkeys = sodium.crypto_kx_client_session_keys(sodium.from_hex(publicK), sodium.from_hex(privateKey), sodium.from_hex(otherPublic));
        const serverkeys = sodium.crypto_kx_server_session_keys(sodium.from_hex(publicK), sodium.from_hex(privateKey), sodium.from_hex(otherPublic));
        map[uid] = {
            hex : otherPublic,
