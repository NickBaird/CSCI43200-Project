		...
		(userCredential) => {
			...
			indexed.onupgradeneeded = function (event) {
				...
				if (!db.objectStoreNames.contains("signature")) {
					const objectStore = db.createObjectStore("signature", {
						keyPath: "uid",
					});
				}
			};
...
async function initialize() {
	...
	display = (
		await database.ref("/users/" + auth.currentUser.uid + "/display").get()
	).val();
	...
	database
		.ref("/users/" + auth.currentUser.uid + "/conversations")
		.on("child_added", (conversation) => {
			add_to_conversations(conversation.key);
		});

	database
		.ref("/users/" + auth.currentUser.uid + "/groups")
		.on("child_added", (group) => {
